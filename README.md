...
```
**3.3.** Если трудно сформулировать имя метода/функции, это *может* быть признаком того, что метод/функция содержит слишком сложную логику и её имеет смысл разделить на несколько более примитивных методов/функций.

## 4. Именование классов
**4.1.** Имена классов должны начинаться с заглавной буквы.

**4.2.** Имя класса должно быть в существительным.

**4.3.** Имя класса должно отражать его роль в системе.
...
**12.1.** В логических выражениях используется только строгое (не)равенство.
```javascript
if (value === 5) { ... }
```
```javascript
if (value !== 10) { ... }
```
**12.2.** Присваивания в логических выражениях не используются.
...
```
**12.3.** Следует избегать [Yoda conditions](http://en.wikipedia.org/wiki/Yoda_conditions).
...
if ('hello' === value) { ... }
```
**12.4.** Допускается использование коротких записей для проверки на ```undefined```, ```null``` или ```0```.
```javascript
if (value) { ... }
```
```javascript
if (!value) { ... }
```
**12.5.** Если два условия, идущих друг за другом, исключают друг друга, нужно использовать конструкцию ```if...else```.
...
```
**12.6.** Не следует использовать логические операторы вместо конструкции ```if...else```.
...
```
**12.7.** Тело выражения всегда заключается в фигурные скобки и переносится на следующую строку.
